...
graph TD
    ...
    Browser -->|":53 UDP/TCP<br>DNS解決"| CoreDNS
    Browser -->|":443 HTTPS"| Traefik

    CoreDNS["CoreDNS<br>:53（ホスト）"]

    subgraph traefik-public["traefik-public ネットワーク"]
        Traefik["Traefik<br>:80, :443（ホスト）<br>Let's Encrypt / Route 53 DNS-01"]
        Portainer["Portainer<br>:9000"]
        ImmichServer["immich-server<br>:2283"]
        Grafana["Grafana<br>:3000"]
    end

    Traefik -->|"portainer.internal<br>.kagiyama.net"| Portainer
    Traefik -->|"immich.internal<br>.kagiyama.net"| ImmichServer
    Traefik -->|"grafana.internal<br>.kagiyama.net"| Grafana

    subgraph immich-internal["immich-internal ネットワーク"]
        ImmichServer
    ...
    subgraph observability-internal["observability-internal ネットワーク"]
        Grafana
